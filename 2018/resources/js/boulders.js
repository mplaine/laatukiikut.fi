...
var boulders = ( function () {
  ...
  var initializeGoogleMaps = function () {
    ...
    for ( var i = 0; i < data.length; i++ ) {
      ...
      var inFinland            = boulder.InFinland;
    ...
    $( '#content-filters-collapse .visible-markers-counter' ).text( markerClusterer.getTotalMarkers() );
    ...
    var votesSliderOptions     = {
      start:    [ Math.min.apply( Math, data.map( function( boulder ) { return boulder.Votes; })) ],
      connect:  'upper',
      tooltips: wNumb({ decimals: 0 }),
      format:   wNumb({ decimals: 0 }),
      range: {
        min:    [ Math.min.apply( Math, data.map( function( boulder ) { return boulder.Votes; })) ],
        max:    [ Math.max.apply( Math, data.map( function( boulder ) { return boulder.Votes; })) ]
      }
    };
    ...
    var gradeSliderOptions     = {
      start:    [ Math.min.apply( Math, data.map( function( boulder ) { return boulder.GradeNumeric; })), Math.max.apply( Math, data.map( function( boulder ) { return boulder.GradeNumeric; })) ],
      connect:  true,
      ...
      format:   wNumb({ decimals: 0 }),
      range: {
        min:    [ Math.min.apply( Math, data.map( function( boulder ) { return boulder.GradeNumeric; })) ],
        max:    [ Math.max.apply( Math, data.map( function( boulder ) { return boulder.GradeNumeric; })) ]
      }
    };
  ...
  var getInfoWindowContent  = function( data ) {
    ...
    var note                = ( data.ApproximateCoordinates === 'Yes' ) ? ' <span class="approximate-location">(pääsyherkkä alue)</span>' : '';
    var location            = ( crag !== '' ) ? crag + ', ' + area : area;
    var commentParagraphs   = '';
    var commentsStr         = data.Comments;
    // Check the existence of comments
    if ( commentsStr.length >= 2 ) {
      // Remove the first and last double quotation mark
      commentsStr           = commentsStr.substr( 1, commentsStr.length - 2 );
      // Comments array
      comments              = commentsStr.split("\",\"");
      // Create the comment paragraphs
      for ( j = 0; j < comments.length; j++ ) {
        commentParagraphs   += '<p class="comment' + (( j === 0 ) ? ' first' : '' ) + '">"' + comments[ j ] + '"</p>';
      }
    }
    ...
    if ( data.ApproximateCoordinates === 'No' ) {
  ...
  var updateMarkerVisibility = function( marker ) {
    ...
    if ( gradeSliderState[ 0 ] <= gradeNumeric && gradeNumeric <= gradeSliderState[ 1 ] ) {
  ...
  var addEventListeners = function () {
    ...
    votesSlider.noUiSlider.on( 'update', function() {
      ...
      if ( votesSliderState !== votesSliderValue ) {
        ...
        $( '#content-filters-collapse .visible-markers-counter' ).text( counter );
      }
    });
